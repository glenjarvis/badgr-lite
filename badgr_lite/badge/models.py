...
class Badge:
    ...
    def _check_missing_but_required(self, pythonic_attrs: dict) -> None:
        ...
        are not given.
        """
        missing_but_required = set(self.REQUIRED_ATTRS) - set(
            pythonic_attrs.keys()
        )
...
class BadgrLite:
    ...
    def badges(self) -> list:
        ...
        """
        self.load_token()
        raw_data = self.get_from_server("https://api.badgr.io/v2/badgeclasses")[
            "result"
        ]
    ...
    def _validate_award_badge_response(self, response: Response) -> None:
        ...
        if response.status_code == 404:
            raise exceptions.BadBadgeIdError(exceptions.BadBadgeIdError.__doc__)

        if response.status_code == 400:
            raise exceptions.AwardBadgeBadDataError(str(response.json()))

        assert (
            response.status_code == 201 and response.json()["status"]["success"]
        )
